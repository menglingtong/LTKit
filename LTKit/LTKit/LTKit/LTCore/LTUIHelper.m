...
@implementation LTUIHelper (Bundle)

+ (NSBundle *)resourcesBundle {
    return [LTUIHelper resourcesBundleWithName:QMUIResourcesMainBundleName];
}
...
+ (UIImage *)imageWithName:(NSString *)name {
    NSBundle *bundle = [LTUIHelper resourcesBundle];
    return [LTUIHelper imageInBundle:bundle withName:name];
}
...
@implementation LTUIHelper (DynamicType)
...
+ (CGFloat)heightForDynamicTypeCell:(NSArray *)heights {
    NSNumber *index = [LTUIHelper preferredContentSizeLevel];
    return [((NSNumber *)[heights objectAtIndex:[index intValue]]) floatValue];
}
@end

@implementation LTUIHelper (Keyboard)

- (void)handleKeyboardWillShow:(NSNotification *)notification {
    self.keyboardVisible = YES;
    self.lastKeyboardHeight = [LTUIHelper keyboardHeightWithNotification:notification];
}
...
+ (BOOL)isKeyboardVisible {
    BOOL visible = [[LTUIHelper sharedInstance] isKeyboardVisible];
    return visible;
}
...
+ (CGFloat)lastKeyboardHeightInApplicationWindowWhenVisible {
    return [[LTUIHelper sharedInstance] lastKeyboardHeight];
}
...
+ (CGFloat)keyboardHeightWithNotification:(NSNotification *)notification {
    return [LTUIHelper keyboardHeightWithNotification:notification inView:nil];
}
...
+ (UIViewAnimationOptions)keyboardAnimationOptionsWithNotification:(NSNotification *)notification {
    UIViewAnimationOptions options = [LTUIHelper keyboardAnimationCurveWithNotification:notification]<<16;
    return options;
}
...
@implementation LTUIHelper (AudioSession)
...
@implementation LTUIHelper (UIGraphic)
...
@implementation LTUIHelper (Device)
...
+ (BOOL)isIPadPro {
    if (isIPadPro < 0) {
        isIPadPro = [LTUIHelper isIPad] ? (DEVICE_WIDTH == 1024 && DEVICE_HEIGHT == 1366 ? 1 : 0) : 0;
    }
    return isIPadPro > 0;
}
...
@implementation LTUIHelper (Orientation)
...
- (void)handleDeviceOrientationNotification:(NSNotification *)notification {
    ...
    [LTUIHelper sharedInstance].orientationBeforeChangingByHelper = UIDeviceOrientationUnknown;
}
...
+ (CGAffineTransform)transformForCurrentInterfaceOrientation {
    return [LTUIHelper transformWithInterfaceOrientation:[[UIApplication sharedApplication] statusBarOrientation]];
}

+ (CGAffineTransform)transformWithInterfaceOrientation:(UIInterfaceOrientation)orientation {
    CGFloat angle = [LTUIHelper angleForTransformWithInterfaceOrientation:orientation];
    return CGAffineTransformMakeRotation(angle);
}

@end

@implementation LTUIHelper (UIApplication)
...
@implementation LTUIHelper (Animation)
...
- (void)printLogWithCalledFunction:(nonnull const char *)func log:(nonnull NSString *)log, ... {
    ...
    if ([self.helperDelegate respondsToSelector:@selector(LTUIHelperPrintLog:)]) {
        [self.helperDelegate LTUIHelperPrintLog:[NSString stringWithFormat:@"QMUI - %@. Called By %s", logString, func]];
    } else {
...
- (void)dealloc {
    // LTUIHelper 若干个分类里有用到消息监听，所以在 dealloc 的时候注销一下
