...
@protocol LTUIHelperDelegate <NSObject>

@required
- (void)LTUIHelperPrintLog:(nonnull NSString *)log;
...
@property(nullable, nonatomic, weak) id<LTUIHelperDelegate> helperDelegate;
...
@interface LTUIHelper (DynamicType)
...
/**
 * 获取当前键盘在屏幕上的可见高度，注意外接键盘（iPad那种）时，[LTUIHelper keyboardRectWithNotification]得到的键盘rect里有一部分是超出屏幕，不可见的，如果直接拿rect的高度来计算就会与意图相悖。
 * @param notification 接收到的键盘事件的UINotification对象
 * @param view 要得到的键盘高度是相对于哪个View的键盘高度，若为nil，则等同于调用[LTUIHelper keyboardHeightWithNotification:]
...
@interface LTUIHelper (AudioSession)
...
@interface LTUIHelper (UIGraphic)
...
@interface LTUIHelper (Orientation)
 ...
 *  @see [LTUIHelper rotateToDeviceOrientation]
 */
...
@interface LTUIHelper (UIApplication)
...
@interface LTUIHelper (Animation)

+ (void)actionSpringAnimationForView:(nonnull UIView *)view;

@end

@interface LTUIHelper (Log)
